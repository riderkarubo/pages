...
| `01b_inspectSlide2.gs` | Phase 0: スライド2のInspect（旧） | ✅ |
| `01c_inspectSlide3.gs` | Phase 0: スライド3のInspect（旧） | ✅ |
| `01d_inspectSlide4.gs` | Phase 0: スライド4のInspect（旧） | ✅ |
| `02_helpers.gs` | 共通ヘルパー（dup/setTitle/txt/rect/makeTable/drawKpiCard等） | ✅ |
| `03_section1.gs` | §1 S1（エグゼクティブサマリ）— P4完全踏襲 | ✅ 実行済み |
| `04_section2.gs` | §2 S2〜S6（2025年度総括・5枚） | コード作成済 |
| `05_section3.gs` | §3 S7〜S12（2026年度KPI・6枚） | コード作成済 |
| `06_section4.gs` | §4 S13〜S19（2本柱・7枚） | コード作成済 |
...
2. `04_section2.gs` を新規ファイルとして追加 → `insertSection2()` を実行（5枚追加）
3. ブラウザで確認 → 修正指示
4. `05_section3.gs` を追加 → `insertSection3()` を実行（6枚追加）
5. ブラウザで確認 → 修正指示
6. `06_section4.gs` を追加 → `insertSection4()` を実行（7枚追加）
7. ブラウザで確認 → 修正指示
